...
起動
 ↓
環境変数の検証（SLACK_WEBHOOK_URL, GEMINI_API_KEY）
 ↓
...
- [×] `src/index.ts` を作成する
- [×] 起動時に `SLACK_WEBHOOK_URL` と `GEMINI_API_KEY` の存在を検証し、未設定なら即時終了する
- [×] 各ステップの開始・完了をログ出力する（`console.info` または構造化ロガー）
- [×] RSSフィードを並列取得し、全記事をマージする
- [×] `filterNewArticles` で未投稿記事を抽出し、先頭5件に絞る
